...
* 1. If app is built locally without any docker or environment variable being injected below, both, _PRODUCTION_URL
...
* If both are injected, _PRODUCTION_URL will have the highest priority and the SERVER_URL will resolve to the injected
* PRODUCTION_URL. Second priority is DEBUG_URL then the default value below.
* */
let _PRODUCTION_URL = 'PRODUCTION_URL' // the String PRODUCTION_URL will be replaced by .gitlab-ci injections if needed. If not replaced, it will be replaced by an empty string below.
let _DEBUG_URL = 'DEBUG_URL'
if (_PRODUCTION_URL.includes('PRODUCTION_UR') && _PRODUCTION_URL.includes('RODUCTION_URL') ) { // hack to prevent the injection to overwrite the test string as well
	_PRODUCTION_URL = ''
}
if (_DEBUG_URL.includes('DEBUG_UR') && _DEBUG_URL.includes('EBUG_URL') ) { // hack to prevent the injection to overwrite the test string as well
	_DEBUG_URL = ''
}

const SERVER_URL = (_PRODUCTION_URL || _DEBUG_URL || 'http://localhost:8000/') // PRODUCTION_URL and DEBUG_URL need to be injected by .gitlab-ci.yml if needed. (Cannot be used as environment variables as this is frontend code which would need the environment variable on the customer's client machine)
