...
def verify_unauthenticated_request_create_fails(self):
	post_request_test_data_and_verify_status_code(
		self=self, test_data=create_request_test_data(), authenticated=False, expected_success=False
	)

def test_request_create_simple_keys_only(self):
...
def test_request_create_multiple_keys_only(self):
	test_data, normalized_test_data = create_request_test_data(
		effects=[1, 2, 5],
		materials=[3, 6],
		lights=[7, 4],
...
def test_request_create_mixed_keys(self):
	test_data, normalized_test_data = create_complex_test_data()
	perform_post_and_retrieve_and_analyze(self, test_data, normalized_test_data)

def verify_authenticated_request_create_succeeds(self):
	test_request_create_simple_keys_only(self)
	test_request_create_multiple_keys_only(self)
	test_request_create_mixed_keys(self)

	# TODO: add preview images and files to createSerializer. (as required)
	# Note: this can be done by posting a file, with a nested Request dict, thus the serializer will do it...
	# Nice benefit: if the file upload is not finished, the request will automatically NOT be posted.

def verify_request_create_ignores_unexpected_argument(self):
	test_data, normalized_test_data = create_request_test_data()
...
def test_request_create_fails_due_missing_argument(self):
	test_data, normalized_test_data = create_request_test_data()
...
def test_request_create_fails_due_invalid_argument_value(self):
	test_data, normalized_test_data = create_request_test_data(
...
def test_request_create_fails_due_pk_of_nonexistent_object(self):
	test_data, normalized_test_data = create_request_test_data(tags=[2, 3, 11, 120, {'tag': 'Create-And-Reuse-Test'}])
	post_request_test_data_and_verify_status_code(
		self=self, test_data=test_data, expected_success=False
	)

def verify_authenticated_request_create_fails(self):
	"""
	...
	"""
	test_request_create_fails_due_missing_argument(self)
	test_request_create_fails_due_invalid_argument_value(self)
	test_request_create_fails_due_pk_of_nonexistent_object(self)
