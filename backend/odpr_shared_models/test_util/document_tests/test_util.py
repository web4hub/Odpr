...
def post_document_test_data_and_verify_status_code(
	...
	request_create_view = DocumentCreateView.as_view()
	if authenticated:
		authenticate_by_user_type(request, user_type)
...
def put_document_test_data_and_verify_status_code(
	self, test_data, authenticated=True, expected_success=True, user_type=AUTHOR
):
	request = make_put_request(data=test_data, format='json')
	request_update_view = DocumentDetailView.as_view()
	if authenticated:
		authenticate_by_user_type(request, user_type)
...
def patch_document_test_data_and_verify_status_code(
	self, test_data, authenticated=True, expected_success=True, user_type=AUTHOR
):
...
def delete_document_instance_and_verify_status_code(
	self, document_id, authenticated=True, expected_success=True, user_type=AUTHOR
):
	request = make_delete_request()
	request_delete_view = DocumentDetailView.as_view()
	if authenticated:
		authenticate_by_user_type(request, user_type)
	response = request_delete_view(request=request, id=document_id)
	verify_response_status_code(self, response, expected_success)
	return response
...
def perform_write_and_verify(
	...
	if 200 <= write_response.status_code < 300:
		response = get_document_test_data_and_verify_status_code(self=self, request_id=write_response.data['id'])
...
def perform_post_and_retrieve_and_analyze(
	...
	return perform_write_and_verify(
		...
		write_function=post_document_test_data_and_verify_status_code, authenticated=authenticated
	)
...
def perform_put_and_retrieve_and_analyze(
	...
	return perform_write_and_verify(
		self=self, test_data=test_data, normalized_test_data=normalized_test_data, expected_success=expected_success,
		write_function=put_document_test_data_and_verify_status_code, authenticated=authenticated,
		user_type=user_type
	)
...
def perform_patch_and_retrieve_and_analyze(
	...
	return perform_write_and_verify(
		self=self, test_data=test_data, normalized_test_data=normalized_test_data, expected_success=expected_success,
		write_function=patch_document_test_data_and_verify_status_code, authenticated=authenticated
	)
