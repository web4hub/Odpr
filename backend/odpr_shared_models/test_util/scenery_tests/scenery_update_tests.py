from odpr_shared_models.test_util.scenery_tests.test_util import perform_post_and_retrieve_and_analyze, perform_put_and_retrieve_and_analyze
from odpr_shared_models.test_util.test_util import AUTHOR, STAFF, ADMIN, VIEWER
from odpr_shared_models.test_util.scenery_tests.test_data import create_complex_test_data, modify_existing_scenery, normalize_list
from odpr_shared_models.models import FilterProperty

def create_and_update_scenery(
	...
	update_test_data, normalized_update_test_data = modify_existing_scenery(response.data)
...
def verify_unauthenticated_scenery_update_fails(self):
	create_and_update_scenery(self=self, authenticated=False, expected_success=False)

def verify_authenticated_other_user_scenery_update_fails(self):
	create_and_update_scenery(self=self, expected_success=False, user_type=VIEWER)

def verify_authenticated_author_scenery_update_succeeds(self):
	create_and_update_scenery(self=self)
...
def verify_authenticated_author_cannot_update_readonly_fields(self):
	...
	update_test_data.update({'download_count': 4000})
	...
	sc = Scenery.objects.get(pk=response.data['id'])
	self.assertNotEqual(sc.visit_count, 5050)
	self.assertNotEqual(sc.download_count, 4000)

def verify_authenticated_staff_can_update_other_user_scenery(self):
	create_and_update_scenery(self=self, user_type=STAFF)

def verify_authenticated_admin_can_update_other_user_scenery(self):
	create_and_update_scenery(self=self, user_type=ADMIN)

def verify_authenticated_author_scenery_update_with_new_elements_succeeds(self):
	test_data, normalized_test_data = create_complex_test_data()
	response = perform_post_and_retrieve_and_analyze(self, test_data, normalized_test_data)
	update_test_data, normalized_update_test_data = modify_existing_scenery(response.data)
	update_test_data['filter_properties'].update({'effects': [{'name': 'New Effect during Update', 'description': {'content': 'Some new effect made during update'}}, 1]})
